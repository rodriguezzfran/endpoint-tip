...
int genericPlus(const void* array, size_t elementSize, size_t fieldOffset, size_t amountOfElements)
{
    ...
    {   
        ...
        // cast to const char* because we need to move the pointer by bytes
        acum += *(const int*)((const char*)array + fieldOffset); // (const int*) dont allow you to modify the value of what is pointed to
        array = (const char*)array + elementSize;
    }
...
structure arrayStructure[9] = {{'a', 1, 1.0, 1.1}, {'b', 2, 2.0, 2.2}, {'c', 3, 3.0, 3.3},
                               {'d', 4, 4.0, 4.4}, {'e', 5, 5.0, 5.5}, {'f', 6, 6.0, 6.6},
                               {'g', 7, 7.0, 7.7}, {'h', 8, 8.0, 8.8}, {'i', 9, 9.0, 9.9}};
